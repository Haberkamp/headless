...
const SliderOrientationContext = createContext<SliderOrientation | null>(null)

export function useSliderOrientationContext(): SliderOrientation {
  const context = useContext(SliderOrientationContext)
  if (!context) {
    throw new Error('SliderOrientation components must be used within SliderHorizontal or SliderVertical')
  }
  return context
}

export function SliderOrientationProvider({ value, children }: { value: SliderOrientation, children: React.ReactNode }) {
  return (
    <SliderOrientationContext.Provider value={value}>
      {children}
